    ...
    */
            ...
            success: function(response) {
                ...
                for (i = 0; i < tmpRetVal.length; i++) {
                    if (!tmpRetVal[i].ffUrl) { // not an object under management by FatFractal - probably an extension result
                        ...
                        var realObj = m_cache[tmpRetVal[i].ffUrl];
                        var grabBagItems = response['grabBagItems'];
                        if (grabBagItems) {
                            if (m_superDebug) console.log("Checking grabBagItems for " + realObj.ffUrl);
                            var grabBagsForThisObj = grabBagItems[realObj.ffUrl];
                            if (grabBagsForThisObj) {
                                if (m_debug) console.log("Found grabBags for " + realObj.ffUrl);
                                for (var grabBagName in grabBagsForThisObj) {
                                    var grabBagEntries = grabBagsForThisObj[grabBagName];
                                    var numEntries = 0;
                                    if (grabBagEntries && grabBagEntries.length)
                                        numEntries = grabBagEntries.length;
                                    if (m_debug) console.log("Found grabBag " + grabBagName + " for " + realObj.ffUrl + " with " + numEntries + " entries");
                                    if (numEntries > 0) {
                                        realObj[grabBagName] = [];
                                        for (var grabBagEntryNum = 0; grabBagEntryNum < numEntries; grabBagEntryNum++) {
                                            var grabBagEntryFfUrl = grabBagEntries[grabBagEntryNum];
                                            if (m_debug) console.log("Adding object " + grabBagEntryFfUrl + " to grabBag " + grabBagName + " for " + realObj.ffUrl);
                                            var grabBagEntryObj = m_cache[grabBagEntryFfUrl];
                                            if (grabBagEntryObj) {
                                                realObj[grabBagName].push(grabBagEntryObj);
                                            }
                                        }
                                    }
                                }
    ...
    function retrieveFFUser() {
        ...
        if (user === null || user === undefined) {
            if (m_debug && console.log) console.log("No existing user info found - creating new FFUser");
            user = new FFUser();
        }
        else {
            if(m_debug && console.log) console.log("Found existing FFUser session info: ", user)
        }
        m_cache[user.ffUrl] = user;
        return user;
    }
